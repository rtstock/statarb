...
##        b = self.setclassdictionaries(closepricesfilepath = closepricesfilepath
##                                      , movingaveragewindow = movingaveragewindow
##                                      , list_of_tickers=list_of_tickers)
        b = self.setclassdictionaries(symbols,fromdate,todate,movingaveragewindow)

    def findopportunities(self
                          ):
        print 'started findopportunities'
        #ticker1 = list_of_tickers[0]
        #ticker2 = list_of_tickers[1]
        ...
        #print 'self.PairRunningPctDiffDictionary',self.PairRunningPctDiffDictionary
        print 'length of dict',len(self.PairRunningPctDiffDictionary)
        for ticker1 in self.SymbolsList:
            
            for ticker2 in self.SymbolsList:
                if not ticker1 == ticker2:
                    print 'finding opportunities',ticker1,ticker2
                    ticker1_closeprice = self.ClosePricesDataframe[-1:][ticker1].iloc[0]
                    ticker2_closeprice = self.ClosePricesDataframe[-1:][ticker2].iloc[0]
                    #stop
                ...
                #try:
                    ...
                    myinput = raw_input("What do you want to do? r to report, enter to continue...")
                    if myinput == 'y':
                        ...
                        df_0.plot.line(title=ticker1 +'='+str(round(ticker1_closeprice,2)) + ' ' + ticker2+'='+str(round(ticker2_closeprice,2)))
...
##            #        break
    ...
    def setclassdictionaries(self,symbols,fromdate,todate,movingaveragewindow):
        
        print 'started def setclassdictionaries'
        import pullprices as pp1
        o = pp1.pull()
        o.setclassdataframes(symbols,fromdate,todate)
        import pandas as pd
        #myfile = closepricesfilepath #'C:\Batches\GitStuff\$work\closeprices_sample.csv'
        #df = pd.read_csv(myfile)
        df = o.AdjClosePricesDataframe
        ...
        for column in columns:
            ...
            for column1 in columns:
                ...
                df_diff_runningmax[column1] = df_diff1.rolling(window=20).max() #df_diff1[column1].cummax().to_frame(column1)
                df_diff_runningmin[column1] = df_diff1.rolling(window=20).min() #df_diff1[column1].cummin().to_frame(column1) #ssss
    ...
    def setclassdictionaries_fromfile(self,closepricesfilepath,movingaveragewindow,list_of_tickers = []):
...
if __name__=='__main__':
    ...
    #closepricesfilepath = 'C:\\Batches\\GitStuff\\$work\\closeprices_sample.csv'
    o = find(
