...
#dict_pairdiff_betweenmaxmin    
    ...
    def __init__(self,
        ...
        pairlist = ['CA','CHD']
        b = self.setclassdictionaries(closepricesfilepath = closepricesfilepath,movingaveragewindow = movingaveragewindow,list_of_tickers=pairlist)
        #self.regressiondifferences(pairlist=pairlist)
        self.analyzepair(pairlist=pairlist,showplot=True)
        
    def analyzepair(self,pairlist,showplot
                    ):
        ...
        startdatetime = datetime.strptime(df.index[0] , '%Y-%m-%d')
        enddatetime = datetime.strptime(df.index[-1], '%Y-%m-%d')
...
##            self.plot_residuals(df,startdatetime,enddatetime)
    ...
    def regressiondifferences(self,):
        ticker1 = pairlist[0]
        ticker2 = pairlist[1]
