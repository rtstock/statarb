...
##        stop
    ...
    def testone(self,ticker1,ticker2,tradedate,triggervalue, maxgain=1000.0):
        ...
        for idx,row in df_blotter.iterrows():
            #print 'a', idx, sign_for_ticker_long * row[ticker1], sign_for_ticker_short * row[ticker2], ( sign_for_ticker_long * row[ticker1] ) + ( sign_for_ticker_short * row[ticker2] ), triggervalue, self.PairPricesDiffDictionary[ticker1][ticker2][idx]
            my_dict = {
                    'date':idx, '02_dollarized1':sign_for_ticker_long * row[ticker1], '03_dollarized2':sign_for_ticker_short * row[ticker2], '04_pl':( sign_for_ticker_long * row[ticker1] ) + ( sign_for_ticker_short * row[ticker2] ), '05_triggervalue':triggervalue, '06_pricediff':self.PairPricesDiffDictionary[ticker1][ticker2][idx]
                    }
        ...
        print df_final.iloc[:1]
            ...
            #        break
        print '-------------------------------------------------'
        print df_final[(df_final['04_pl'] >= maxgain)].iloc[:1]
        stop
...
##        stop
    ...
    def testone_old(self,ticker1,ticker2,tradedate,triggervalue):
        ...
        for idx,row in df_blotter.iterrows():
            print 'a', idx, sign_for_ticker_long * row[ticker1],sign_for_ticker_short * row[ticker2], ( sign_for_ticker_long * row[ticker1] ) + ( sign_for_ticker_short * row[ticker2] )
            #if idx > '2016-03-02':
            #        break
        print '-------------------------------------------------'
##        for idx,row in df1a.iterrows():
