...
#SlopeOfPairsDifferenceDataframe
    def set_SlopeOfPairsDifferenceDataframe(self,SlopeOfPairsDifferenceDataframe):
        self._SlopeOfPairsDifferenceDataframe = SlopeOfPairsDifferenceDataframe
    def get_SlopeOfPairsDifferenceDataframe(self):
        return self._SlopeOfPairsDifferenceDataframe
    SlopeOfPairsDifferenceDataframe = property(get_SlopeOfPairsDifferenceDataframe, set_SlopeOfPairsDifferenceDataframe)
...
#dollarizedhedgeratiodataframes
    ...
    def __init__(self                 
        ...
        b = self.setslopeofpairsdifferencedataframe()
    ...
    def setslopeofpairsdifferencedataframe(self,
                                        ):
        df = self.ClosePricesDataframe
        #df_analyze = self.PairPricesDiffDictionary
        #df_analyze = self.PairDollarizedHedgeRatioDictionary
        #df_analyze = self.PairDollarizedHedgeRatioDiffDictionary
        df_analyze = self.PairDollarizedDiffDictionary
        df_slope = pd.DataFrame(index = self.SymbolsList)
        columns = self.SymbolsList
        for s1 in columns:
            df_pp = pd.DataFrame(index = df.index)
            if not s1 in df_analyze:
                print s1,'not found'
            else:
                print 'Doing',s1
                df_diff = df_analyze[s1] 
        ...
        self.SlopeOfPairsDifferenceDataframe = df_slope
        return True


    def setslopeofpairsdifferencedataframe_old(self,
                                        ):
        df = self.ClosePricesDataframe
        df_slope = pd.DataFrame(index = self.SymbolsList)
        columns = self.SymbolsList
...
##                        #stop
    ...
    def setclassdictionaries(self,list_of_tickers = [],closepricesfilepath = '',fromdate = '2017-01-01',todate = '2017-12-31'):
        ...
        df_analyze = df.multiply(df_shares3, axis=1)
        ...
        for column in columns:
            ...
            df_diff_dollarized = df_analyze[columns].sub(df_analyze[column], axis=0)
...
##        self.BetaHedgeRatioDataframe = df_betahr
    ...
    print 'done',b
    print '----------------'
    print 'df_slope diffs'
    print o.SlopeOfDiffsDataframe.round(3)
    print 'df_slope pairs diff'
    print o.SlopeOfPairsDifferenceDataframe.round(3)
    #stop
...
##        print v
    ...
    print 'SlopeOfPairsDifferenceDataframe'
    ...
    print o.SlopeOfPairsDifferenceDataframe
    ticker1 = 'PCLN'
    ticker2 = 'MSFT'
...
##    d2['df_betahr'].to_csv(cachedfilepathname_betahr,columns=(list(d2['df_betahr'].columns.values)))
    ...
    #o.SlopeOfPairsDifferenceDataframe.to_csv(cachedfilepathname_SlopeOfDollarizedHedgeRatio,columns=(list(o.SlopeOfPairsDifferenceDataframe.columns.values)))
